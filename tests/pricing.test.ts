...
test("rateForModel returns OpenRouter-sourced rates for known models", () => {
  ...
  assert.equal(rateForModel("z-ai/glm-5.2")?.input, 0.9548);
  assert.equal(rateForModel("/data/models/hf/zai-org__GLM-5.2-FP8")?.input, 0.9548);
...
test("pricing resolves the listed model instead of a broad GPT or Claude family guess", () => {
  ...
  assert.deepEqual(rateForModel("z-ai/glm-5.2"), { input: 0.9548, output: 3.0008, cacheRead: 0.17732, cacheWrite: 0.9548 });
