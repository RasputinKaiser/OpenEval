...
export function CompareCharts({ rows, onTransition, onCase }: { rows: ComparisonValues[]; onTransition: (key: string) => void; onCase: (key: string) => void }) {
  ...
  const setMetricFilter = (next: ComparisonMetric) => { setMetric(next); const url = new URL(window.location.href); url.searchParams.set("cmpMetric", next); window.history.pushState(window.history.state, "", url); };
  ...
  const format = (value: number) => `${value > 0 ? "+" : ""}${metric === "cost" ? "$" : ""}${Number(value.toFixed(metric === "cost" ? 6 : metric === "rate" ? 2 : 0))}`;
  return <div className="grid gap-3 mb-4 lg:grid-cols-2">
    ...
    <ChartFrame title="Per-case metric changes" description="Expand to inspect B minus A for the same case and sample. Colors describe direction, not overall evaluation quality." unit="Missing or unmatched metrics are excluded from bars and retained in the evidence table."
      details={<><label className="text-xs text-fg-muted">Metric <select className="analysis-input mb-3" value={metric} onChange={(event) => setMetricFilter(event.target.value as ComparisonMetric)}><option value="cost">Cost (USD)</option><option value="rate">Output tokens / second</option><option value="turns">Turns</option></select></label><p className="text-xs text-fg-muted mb-2">{deltas.length} / {rows.length} pairs have both values; {rows.length - deltas.length} unavailable.</p><SelectableBars rows={deltas} format={format} noun="case and sample" onExplore={onCase} /></>}
      ...
      <p className="text-xs text-fg-muted">{rows.length} case/sample pairs. Expand the metric view or open the data table to inspect exact changes.</p>
    </ChartFrame>
  </div>;
}
