...
export function MetricDistribution({ title, values, format = String, unit, description }: {
  ...
  return <ChartFrame title={title} description={description} unit={unit}
    ...
    {bins.length ? <div>
      <div className="flex items-end gap-1 h-32 overflow-x-auto" role="group" aria-label={`${title} bins`} onKeyDown={event => { if (event.key === "Escape") setSelected(null); }}>
        {bins.map((bin, i) => <button key={i} type="button" className="distribution-bin flex-1 min-w-10 h-full flex flex-col justify-end rounded-t focus-visible:outline-offset-2" style={{ transform: "none" }} aria-pressed={selected === `${bin.min}:${bin.max}`} aria-label={`${format(bin.min)} to ${format(bin.max)}: ${bin.count}`} onMouseMove={event => show(event, `${format(bin.min)} inclusive to ${format(bin.max)} ${bin.final ? "inclusive" : "exclusive"}: ${bin.count} observations`)} onMouseLeave={hide} onFocus={event => showAt(event.currentTarget, `${format(bin.min)} inclusive to ${format(bin.max)} ${bin.final ? "inclusive" : "exclusive"}: ${bin.count} observations`)} onBlur={hide} onClick={() => setSelected(selected === `${bin.min}:${bin.max}` ? null : `${bin.min}:${bin.max}`)}>
