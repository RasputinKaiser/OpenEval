 ...
 */
export default function SupportLinks({ collapsed = false, headingId = "support-links-title" }: { collapsed?: boolean; headingId?: string }) {
  return (
    <section aria-labelledby={headingId} className="support-links px-3 py-3">
      <div className={clsx("mb-2", collapsed && "md:sr-only")}>
        ...
        <p className="mt-1 text-[10px] leading-4 text-fg-dim">
          OpenEval is free and local-first. Support is optional.
        </p>
      </div>
      <nav aria-label="Support and contact links" className="space-y-0.5">
        {SUPPORT_LINKS.map(({ id, label, href, icon: Icon }) => (
          ...
          >
            ...
            <span className={clsx("min-w-0 truncate", collapsed && "md:sr-only")}>{label}</span>
            <ExternalLink aria-hidden="true" className={clsx("ml-auto size-3 shrink-0 text-fg-dim", collapsed && "md:hidden")} />
          </a>
        ))}
      </nav>
    </section>
